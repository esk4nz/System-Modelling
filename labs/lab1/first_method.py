...
if __name__ == "__main__":
    ...
    for lam in lambdas:
        ...
        chi_stat, chi_crit, df, decision, k_eff = chi_square_test_exp_equal_prob(
        ...
        plt.title(f"Гістограма частот (λ={lam:g}, n={n})")
        plt.xlabel("x")
        ...
        if SHOW_THEORETICAL:
            ...
            plt.plot(xs, pdf, "r-", lw=2, label="Теоретична щільність λe^(-λx)")
            plt.xlim(0, x_max_plot)

        plt.title(f"Експоненційний розподіл (λ={lam:g}, n={n})")
        plt.xlabel("x")
