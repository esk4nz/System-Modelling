...
def normal_sample(n, a=0.0, sigma=1.0):
    """
    Генерує n випадкових чисел з нормальним розподілом N(a, sigma^2)
    за методом суми 12 рівномірних чисел.
    """
...
def chi_square_test_norm(x, a, sigma, k=20, alpha=0.05):
    """
    χ²-тест узгодженості для нормального розподілу з відомими параметрами.
    Інтервали розраховані так, щоб мати приблизно рівні ймовірності.
    """
    n = len(x)
    if n == 0:
        raise ValueError("Порожня вибірка")

    # Межі бінів за квантилями
    ...
    # Злиття бінів із малими очікуваними частотами
    obs_m, exp_m = [], []
    acc_o = acc_e = 0.0
...
if __name__ == "__main__":
    ...
    for a, sigma in params:
        ...
        chi_stat, chi_crit, df, p_value, decision, k_eff = chi_square_test_norm(
        ...
        plt.hist(
            ...
            color="tab:green", edgecolor="black", linewidth=0.4,
        ...
        plt.title(f"Нормальний розподіл (a={a}, σ={sigma}, n={n}, df={df}, k_eff={k_eff})")
        plt.xlabel("x")
